...
export default function CameraModal({ onCapture, inventory, setInventory, showNotification, currentUser }) {
  ...
  const sendPhoto = async () => {
    ...
    try {
      await onCapture?.(photoFile, (inferredCart) => {
        console.log(inferredCart);
        setCart(inferredCart);
      ...
      {step === STEPS.CAMERA && (
        <>
          ...
          <div className="relative z-10 flex flex-col h-full">
            {/* Top bar */}
            <div className="flex justify-between items-center p-5 pt-5">
              <button
                onClick={closeModal}
                className="bg-black/40 backdrop-blur-sm p-2 rounded-full text-white"
              >
                <X size={22} />
              </button>
            ...
            {/* Hint text */}
            <p className="text-center text-white/80 text-sm mb-4 px-8">
              Apunta la cámara hacia la lista de ventas
            </p>
            ...
            <div className="flex justify-center items-center pb-12 gap-16">
              ...
              <button
                onClick={takePhoto}
                className="w-15 h-15 rounded-full bg-white flex items-center justify-center shadow-lg active:scale-95 transition-transform"
              >
        ...
        <>
          ...
          <div className="relative z-10 flex flex-col h-full">
            ...
            <div className="p-5 pb-12 flex items-center justify-between gap-40">
              ...
              <button
                onClick={sendPhoto}
                className="flex-1 bg-[#0796c2] hover:bg-[#006b9e] text-white font-semibold py-3 rounded-3xl flex items-center justify-center gap-2 text-base shadow-lg active:scale-95 transition-transform"
              >
                Analizar
              </button>
            </div>
        ...
        <>
          ...
          <div className="relative z-10 flex-1 flex flex-col items-center justify-center gap-6 px-8">
            <div className="text-center">
              <p className="text-white/60 text-m mt-2">Identificando productos y cantidades</p>
            </div>
      ...
      {step === STEPS.RESULT && (
        <div className="flex flex-col h-full">
          ...
          <div className="flex justify-between items-center p-5 pt-10 shrink-0">
            <button onClick={closeModal} className="text-white/70">
              <X size={22} />
            </button>
              ...
              <div key={i} className="bg-white/10 backdrop-blur-sm rounded-xl px-4 py-3 flex justify-between items-center">
                ...
                <div className="text-right">
                  ...
                  <p className="text-white/40 text-xs">
                    x{item.cantidadVendida} = ${(item.precio * item.cantidadVendida).toFixed(2)}
                  </p>
                </div>
              </div>
            ))}
          ...
          <div className="p-5 pb-10 flex gap-30 shrink-0">
            <button
              onClick={retake}
              className="flex items-center gap-2 px-4 py-3 rounded-3xl border border-white/30 text-white text-sm"
            >
              <RotateCcw size={16} />
              Repetir
            </button>
            <button
              ...
              className="flex-1 bg-green-500 hover:bg-green-600 text-white font-semibold py-3 rounded-3xl flex items-center justify-center gap-2 active:scale-95 transition-transform"
            >
              <Send size={20} />
            </button>
          </div>
